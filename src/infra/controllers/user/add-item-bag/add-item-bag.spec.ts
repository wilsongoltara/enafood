import { InMemoryAddItemToBagRepository, InMemoryUpdateUserRepository } from "test/repositories/in-memory-user-repository";
...
test('add item in bag', async () => {
  ...
  const addItemToBagController = new AddItemToBagController(inMemoryAddItemToBagRepository);
...
test('No add item with invalid props', async () => {
  ...
  expect(statusCode).toEqual(400);
  expect(body).toEqual('Field quantity is required');
});